...
def write(file_format, filename, num_datasets, dimensions):
 ...
 # Create datasets and populate them with data
    for i in range(0, num_datasets):
        ...
        #FIXME: the compression across file formats are not consistent now (06/23/2023)
        ...
        if file_format == 'HDF5_compressed':
            t1 = time.perf_counter()
            dataset = file.create_dataset(f'Dataset_{i}', shape=dimensions, dtype='f', compression='gzip')
        if file_format == 'Zarr':
            t1 = time.perf_counter()
            dataset = file.create_dataset(f'Dataset_{i}', shape=dimensions, dtype='f')
        ...
        if file_format == 'netCDF4':
            ...
            t1 = time.perf_counter()
            dataset = file.createVariable(f'Dataset_{i}', dimensions=axes, datatype='f', compression='gzip')
