...
def plot(file_formats, num_datasets, dimensions):
    ...
    if not os.path.exists('unit_test/plot_files'):
        os.mkdir('unit_test/plot_files')
    ...
    for i in range(0, len(file_formats)):
        # Round to 3 decimal places so data shows nicely
    ...
    plt.tight_layout()
    plt.savefig(f'unit_test/plot_files/{num_datasets}_{dimensions}_read_write.png')
    plt.cla()
    plt.clf()
    ...
    plt.xticks(x, plt_labels)
    for i in range(0, len(file_formats)):
        # Round to 3 decimal places, so that it displays nicely on the plot.
        ...
        plt.bar_label(bar_read_write, padding=2, fontsize=7)
        offset += width
    plt.legend(fontsize=7, loc='upper center')
    plt.savefig(f'unit_test/plot_files/{num_datasets}_{dimensions}_create_open.png')
    plt.cla()
    plt.clf()
...
def process_csv(file_formats, num_datasets, dimensions):
    ...
    for file_format in file_formats:
        df = pd.read_csv(f'unit_test/csv_file/{file_format}_{num_datasets}_{dimensions}.csv')
        ...
        df.to_csv(f'unit_test/csv_file/{file_format}_{num_datasets}_{dimensions}.csv', index=False)
